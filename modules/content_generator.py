...
class ContentGenerator:
    ...
    def _validate_url(self, url: str) -> bool:
        """Valida si la URL es válida y pertenece a Tiendanube"""
        if not url:
            return False
        try:
            ...
            return response.status_code == 200 and 'tiendanube.com' in url.lower()
        except Exception:
            return False

    def _analyze_keywords(self, texto: str) -> List[str]:
        """Analiza y extrae palabras clave relevantes del texto"""
        if not texto or len(texto.strip()) < 10:
            return []

        prompt = f"""Analiza este texto y extrae las 5-8 palabras clave más relevantes para SEO.
Consideraciones:
- Incluir palabras clave long-tail
- Priorizar términos de búsqueda comercial
- Considerar la intención del usuario
...
Texto: {texto}"""
        ...
        try:
            ...
            keywords = response[0]['generated_text'].split(',')
            # Filtrar y limpiar keywords
            cleaned_keywords = []
            for k in keywords:
                k = k.strip().lower()
                if k and len(k) > 2 and k not in cleaned_keywords:
                    cleaned_keywords.append(k)
            return cleaned_keywords[:8]  # Limitar a máximo 8 keywords
        except Exception as e:
            print(f"Error en análisis de keywords: {str(e)}")
            return []
